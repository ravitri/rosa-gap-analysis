...
create_pr() {
    ...
    if ! rsync -av --exclude='.git' "${source_dir}/" ./ 2>&1 | grep -v "sending incremental file list"; then
        log_error "Failed to copy generated files from ${source_dir}"
        exit 1
    fi
    log_success "✓ Files copied"

    # Run make to generate additional files (REQUIRED for managed-cluster-config)
    ...
    if [ -f Makefile ] || [ -f makefile ]; then
        ...
        if make 2>&1 | tee /tmp/make-output.log; then
            ...
            # Show what files were generated/modified by make
            ...
            log_error ""
            log_error "The PR cannot be created because 'make' failed."
            log_error "This usually indicates missing dependencies or invalid generated files."
            exit 1
        fi
    else
        log_warn "No Makefile found in repository - skipping make step"
        log_warn "This is unusual for managed-cluster-config repository"
    fi

    # Stage all changed files (gap-analysis files + make-generated files)
    log_info "Staging all changed files (gap-analysis + make-generated)..."
    if ! git add -A; then
    ...
    # Verify that working tree is clean after staging (critical for CI check)
    log_info "Verifying all changes are staged..."
    local unstaged_changes
    unstaged_changes=$(git status --short 2>/dev/null | grep -E "^ M|^ D|^\?\?" || true)
    if [ -n "${unstaged_changes}" ]; then
        log_error "Found unstaged changes after 'git add -A'!"
        log_error "This should not happen. Unstaged files:"
        echo "${unstaged_changes}" >&2
        log_error ""
        log_error "This indicates a problem with the file staging process."
        exit 1
    fi
    log_success "✓ Working tree is clean (all changes staged)"
...
Co-Authored-By: ${GIT_USER_NAME} <${GIT_USER_EMAIL}}"
    ...
    if [ -f Makefile ] || [ -f makefile ]; then
        # Run make again to verify no changes occur
        if make 2>&1 > /tmp/make-verify.log; then
            # Check if any files changed
            local changes_after_make
            changes_after_make=$(git status --short 2>/dev/null || true)
            if [ -n "${changes_after_make}" ]; then
                log_error "'make' is not idempotent - running it again produced changes!"
                log_error "This will cause the CI pr-check to fail."
                log_error ""
                log_error "Changed files:"
                git status --short
                log_error ""
                log_error "This indicates the Makefile has non-deterministic behavior or"
                log_error "depends on external state that differs between environments."
                log_error "The PR cannot be created until this is resolved."
                exit 1
            fi
            log_success "✓ Make is idempotent (no changes on re-run)"
        else
            log_error "Make verification failed on second run!"
            log_error "Output:"
            cat /tmp/make-verify.log >&2
            exit 1
        fi
    fi
